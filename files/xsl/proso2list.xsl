...
<xsl:transform
  ...
  <xsl:template match="/">
    ...
    <xsl:result-document href="#title" method="ixsl:replace-content">
      <xsl:text>Liste des entités prosopographiques (n="</xsl:text>
      <xsl:value-of select="count(//eac:eac-cpf)"></xsl:value-of>
      <xsl:text>")</xsl:text>
    </xsl:result-document> 
    ...
    <xsl:result-document href="#experts">
      <table id="experts-table">
        <thead>
          <tr><th>Id</th><th>Nom</th><th>Naissance</th><th>Décès</th><th>Catégorie</th><th>Actions</th></tr>
        </thead>
        <tbody>
      <xsl:for-each select="//eac:eac-cpf">
        <tr>
          <td>
          ...
          <td><xsl:value-of select="normalize-space(eac:cpfDescription/eac:description/eac:existDates/eac:dateRange/eac:toDate/@standardDate)"/></td>
          <td>
            <xsl:value-of select="normalize-space(eac:cpfDescription/eac:identity/eac:entityType)"/>
          </td>
          <td><a href="{'/xpr/biographies/' || @xml:id || '/view'}">Voir</a> | <a href="{'/xpr/biographies/' || @xml:id || '/modify'}">Modifier</a></td>
        </tr>
      </xsl:for-each>
        </tbody>
      </table>
